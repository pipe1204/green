...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function ensureFormat(
      fmt: { internalFormat: number; format: number } | null,
      fallbackInternal: number,
      fallbackFormat: number
    ): { internalFormat: number; format: number } {
      return (
        fmt ?? { internalFormat: fallbackInternal, format: fallbackFormat }
      );
    }
    ...
    function initFramebuffers() {
      ...
      const GL2 = "drawBuffers" in gl ? (gl as WebGL2RenderingContext) : null;
      const RGBA = GL.RGBA;
      const RG = GL2 ? GL2.RG : RGBA;
      const RED = GL2 ? GL2.RED : RGBA;
      const rgba = ensureFormat(ext.formatRGBA, RGBA, RGBA);
      const rg = ensureFormat(ext.formatRG, RG, RG);
      const r = ensureFormat(ext.formatR, RED, RED);
